...
In addition to the quality of life improvements described in this section,
it is possible to get shell completions in `bash` or `zsh` with [`argcomplete`](https://docs.ansible.com/projects/ansible/devel/installation_guide/intro_installation.html#shell-completion).
To add, simply execute the instructions shown in @fig-argcomplete.
...
Instructions for the installation of argcomplete.
It can be helpful to add this to execution environments since it might not be
desirable to keep these dependencies on the host.
...
```{.yaml include="../execution-environment.yaml"}
...
Additionally, it is possible to configure the navigator to have a default
entrypoint using `ansible-navigator.yaml` as in @fig-navigator-config.
...
How to execute an arbitrary playbook with `ansible-navigator` _(in this case it is `test.yaml`)_.

:::

::: { #fig-navigator-config }

```{ .yaml include="../ansible-navigator.yaml"}

```

Default entrypoint for `ansible-navigator` for this project. This can be invoked with
`ansible-navigator exec` - it is also possible to obtain a shell within the
execution environment like `ansible-navigator exec /bin/bash` _(though the shell
might change depending on the base image used.)_.
